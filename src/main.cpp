...
struct uint24_t {
    uint32_t value : 24; // 24-bit field

    // Constructor for easy initialization
    uint24_t(uint32_t val = 0) : value(val >> 8) {}

    // Assignment operator
    uint24_t& operator=(uint32_t val) {
        value = val >> 8; // Ensure only the lower 24 bits are stored
        return *this;
    }
...
void img_save(uint32_t* pixels, int width, int height)
{
    ...
    if (ImGui::Button("Save file as")) {
        ...
        if (ext == "bmp") {
            ...
            img_save_kips(pixels, filepath.c_str(), width, height);
            std::cout << "Imagem salva com sucesso em: " << filepath << std::endl;
        } else {
...
std::string get_extension(const char* filename) {
    ...
    std::string::size_type idx = file_str.rfind('.');
    if (idx != std::string::npos) {
        return file_str.substr(idx + 1);
    }
...
palette k_means(float** in, uint8_t* out, size_t k, size_t data_size, size_t data_depth=3, int seed=69, size_t max_iter=200)
{
    ...
    // 3. recompute centroids by taking the mean between all points it contains 
    // 4. repeat steps 2 and 3 until convergence (i.e. when the step 3 makes no significant changes to the centroids) or until max_steps
